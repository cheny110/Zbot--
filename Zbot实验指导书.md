<!-- vscode-markdown-toc -->
* 1. [搭建开发环境](#)
	* 1.1. [VS Code 开发环境](#VSCode)
	* 1.2. [VNC远程桌面工具](#VNC)
* 2. [机器人启动与参数配置](#-1)
	* 2.1. [启动Zbot3 底盘](#Zbot3)
	* 2.2. [EKF简介](#EKF)
	* 2.3. [启动雷达](#-1)
	* 2.4. [启动相机](#-1)
	* 2.5. [查看IMU话题信息](#IMU)
...
* 3. [机器人遥控与可视化](#-1)
	...
	* 3.3. [手机App 遥控](#App)
		* 3.3.1. [APP蓝牙连接](#APP)
		* 3.3.2. [APP 遥控方式简介](#APP-1)
* 4. [PID 调参](#PID)
* 5. [控制器通信协议说明](#-1)
	* 5.1. [香橙派->驱动板控制协议](#-)
	* 5.2. [驱动板->香橙派数据协议](#--1)
* 6. [开塔机器人操作实验](#-1)
	* 6.1. [查看kata机器人urdf模型](#kataurdf)
	* 6.2. [ROS中的服务通信机制](#ROS)
	* 6.3. [在节点中添加服务提供](#-1)
	* 6.4. [在节点中创建服务请求端](#-1)
	* 6.5. [URDF 简介](#URDF)
...
* 9. [Slam 建图实验](#Slam)
	* 9.1. [SLAM建图算法](#SLAM)
	* 9.2. [Gmapping 配置文件一览](#Gmapping)
	* 9.3. [保存地图](#-1)
	* 9.4. [使用hector建图](#hector)
	* 9.5. [使用Cartograpger 建图](#Cartograpger)
* 10. [Zbot3 导航](#Zbot3-1)
	* 10.1. [配置movebase](#movebase)
	* 10.2. [配置amcl](#amcl)
	* 10.3. [使用map_server 加载地图](#map_server)
	* 10.4. [运行zbot3 导航](#zbot3)
	* 10.5. [实现多点往复导航](#-1)
	* 10.6. [实现小车自主探索建图](#-1)
		* 10.6.1. [启动explore_lite](#explore_lite)
		* 10.6.2. [使用RRT-Exploration建图](#RRT-Exploration)
* 11. [Zbot3 视觉SLAM建图](#Zbot3SLAM)
	* 11.1. [运行Zbot3 视觉Slam建图](#Zbot3Slam)
* 12. [Zbot follow 功能包功能说明和使用](#Zbotfollow)
* 13. [Zbot Monitor 使用说明](#ZbotMonitor)
	* 13.1. [Zbot 语音控制](#Zbot)
	* 13.2. [Zbot Monitor 开塔机器人面板](#ZbotMonitor-1)
* 14. [开塔机器人快速上手](#-1)
* 15. [Zbot手势控制](#Zbot-1)
* 16. [Zbot 货物搬运实验](#Zbot-1)
...
###  6.1. <a name='kataurdf'></a>查看kata机器人urdf模型
...
类型进行选择，例如，对于激光雷达，可以使用基于激光束的传感器模型：
...
Zbot通讯配置可以方便用户获取Zbot当前网络下的IP地址以及配置ROS多机通信功能。三栏分别对应ROS多机通信中的**ROS_IP**, **ROS_MASTER_URI**,**ROS_MASTER_NAME**三个环境变量。
中间两个选择框：
...
Zbot3的语音控制功能集成在Zbot Moitor 程序上。参考上面Zbot Monitor使用教程。其次，其还依赖讯飞语音识别API 密匙。需要用户前往科大讯飞开放平台平台申请。
[讯飞开放平台](https://passport.xfyun.cn/login)
...
##  16. <a name='Zbot-1'></a>Zbot 货物搬运实验
